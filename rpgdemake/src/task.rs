...
impl TryFrom<Task<Create>> for Task<Validate> {
    ...
    #[tracing::instrument(skip_all)]
    fn try_from(prev: Task<Create>) -> anyhow::Result<Self> {
        debug!("task validate");
...
impl Validate {
    ...
    #[tracing::instrument]
    pub fn map_extension(input: &str) -> Option<&'static str> {
        match input {
    ...
    fn validate_header(file: &Path) -> anyhow::Result<()> {
        use crate::lore::{
            ENCRYPTED_PART_LEN, RPG_HEADER, RPG_HEADER_LEN,
        };
...
impl TryFrom<Task<Validate>> for Task<Decrypt> {
    ...
    #[tracing::instrument(skip_all)]
    fn try_from(prev: Task<Validate>) -> anyhow::Result<Self> {
        debug!("task decrypt");
...
impl TryFrom<Task<Decrypt>> for Task<Write> {
    ...
    #[tracing::instrument(skip_all)]
    fn try_from(prev: Task<Decrypt>) -> anyhow::Result<Self> {
        debug!("task write");
...
impl TryFrom<Task<Write>> for Task<Done> {
    ...
    #[tracing::instrument(skip_all)]
    fn try_from(prev: Task<Write>) -> anyhow::Result<Self> {
