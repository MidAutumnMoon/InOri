        ...
        if (pixelCount > 10000) {
            ...
            for (int i = 0; i < taskCount; ++i) {
                ...
                if (processType == ProcessType.ENCRYPT) {
                    task = () -> {
                        if (begin >= loopPosition) {
                            for (int j = begin; j < end; ++j) {
                                newPixels[positions[j - loopPosition]] = pixels[
                                    positions[j]
                                ];
                            }
                        } else if (end <= loopPosition) {
                            for (int j = begin; j < end; ++j) {
                                newPixels[positions[j + offset]] = pixels[
                                    positions[j]
                                ];
                            }
                        } else {
                    ...
                    task = () -> {
                        if (begin >= loopPosition) {
                            for (int j = begin; j < end; ++j) {
                                newPixels[positions[j]] = pixels[
                                    positions[j - loopPosition]
                                ];
                            }
                        } else if (end <= loopPosition) {
                            for (int j = begin; j < end; ++j) {
            ...
            if (processType == ProcessType.ENCRYPT) {
                ...
                for (int i = loopPosition; i < pixelCount; ++i) {
                    newPixels[positions[i - loopPosition]] = pixels[
                        positions[i]
                    ];
                }
            } else {
                ...
                for (int i = loopPosition; i < pixelCount; ++i) {
                    newPixels[positions[i]] = pixels[
                        positions[i - loopPosition]
                    ];
                }
            }
        }
