...
impl Naguru {
    ...
    async fn exec_args(
        &self,
        cmd_path: String,
        // idiot kwin script can't do "sas"
        ...
        let res: Result<(), String> = 'out: {
            ...
            {
                break 'out Err(
                    "The given command is not from nix store".into()
                );
            }
            ...
            let args = {
                ...
                for a in args {
                    ...
                    if let Str(s) = a.into() {
                        ...
                        break 'out Err(
                            "Sig is not sav, but in fact sas. \
                            DBus sucks so here's the workaround."
                                .into(),
                        );
                    }
                }
