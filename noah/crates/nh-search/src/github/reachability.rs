...
pub use super::types::{
    BranchReachability, BranchReachabilityRequest,
...
pub(super) fn probe(
    ...
    for group in
        requests.chunks(COMPARE_CHUNK_SIZE * MAX_PARALLEL_COMPARE_CHUNKS)
    {
        let mut group_results = thread::scope(|scope| {
            let mut handles = Vec::new();
            for chunk in group.chunks(COMPARE_CHUNK_SIZE) {
                handles
                    .push(scope.spawn(move || probe_chunk(client, chunk)));
            }
        ...
        Err(err) => {
            ...
            requests
                ...
                .map(|request| BranchReachability {
                    ...
                    status: BranchReachabilityStatus::Unknown(
                        reason.clone(),
                    ),
                })
                .collect()
...
fn compare_query(
    ...
    for (index, request) in requests.iter().enumerate() {
        ...
        definitions.push(format!(
            "${branch_var}: String!, ${commit_var}: String!"
        ));
        fields.push(format!(
...
fn compare_status_to_reachability(
    status: &str,
) -> BranchReachabilityStatus {
    match status {
...
mod tests {
    ...
    use super::{
        BranchReachabilityRequest, BranchReachabilityStatus,
        compare_query, compare_status_to_reachability, error_chain,
    };
    ...
    fn unknown_reason_includes_error_chain() {
        ...
        let error = eyre!("transport timed out")
