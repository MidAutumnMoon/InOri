...
mod tests {
    ...
    fn darwin_scope_maps_to_darwin_option_type() {
        assert_eq!(
            option_scope_types(args::OptionScope::Darwin),
            &[TYPE_DARWIN_OPTION]
        );
        assert_eq!(
            option_scope_label(args::OptionScope::Darwin),
            "nix-darwin"
        );
    }
    ...
    fn all_scope_includes_darwin_option_type() {
        assert!(
            option_scope_types(args::OptionScope::All)
                .contains(&TYPE_DARWIN_OPTION)
        );
    }
}
