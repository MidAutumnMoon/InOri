...
pub fn run(
    ...
    for db_path in databases {
        ...
        match db {
            SpamDb::Options(opts_db) => {
                let records =
                    opts_db.query(&query_s).with_context(|| {
                        format!(
                            "querying options database: {}",
                            db_path.display()
                        )
                    })?;
                for rec in records {
            ...
            SpamDb::Packages(pkgs_db) => {
                let records =
                    pkgs_db.query(&query_s).with_context(|| {
                        format!(
                            "querying packages database: {}",
                            db_path.display()
                        )
                    })?;
                for rec in records {
    ...
    let has_results =
        !option_results.is_empty() || !package_results.is_empty();
    ...
    for (db_path, rec) in &package_results {
        ...
        if !rec.packages.is_empty() {
            ...
            let lines =
                textwrap::wrap(&pkgs, textwrap::Options::with_termwidth());
...
fn fair_split(
    ...
    let opt_take = opt_take
        + (limit - opt_take - pkg_take).min(option_len - opt_take);
    (opt_take, pkg_take)
}
