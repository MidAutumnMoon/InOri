...
fn generate_single(
    ...
    match shell {
        CompletionShell::Bash => generate_to(
        ...
        CompletionShell::Elvish => generate_to(
        ...
        CompletionShell::Fish => generate_to(
        ...
        CompletionShell::PowerShell => generate_to(
        ...
        .map_err(|e| {
            format!("Failed to generate PowerShell completion: {e}")
        }),
        CompletionShell::Zsh => generate_to(
        ...
        .and_then(|path| {
            let new_path =
                path.with_file_name(format!("{BINARY_NAME}.zsh"));
            std::fs::rename(&path, &new_path).map_err(|e| {
                format!("Failed to rename Zsh completion: {e}")
            })?;
            Ok(new_path)
        }),
        CompletionShell::Nushell => generate_to(
        ...
        .map_err(|e| {
            format!("Failed to generate Nushell completion: {e}")
        }),
    }
}
