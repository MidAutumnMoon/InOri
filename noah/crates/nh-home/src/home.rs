        ...
        match self.subcommand {
            ...
            HomeSubcommand::Build(args) => {
                if args.common.ask || args.common.dry {
                    warn!(
                        "`--ask` and `--dry` have no effect for `nh home build`"
                    );
                }
                args.rebuild(&Build)
            }
...
impl HomeRebuildArgs {
    fn rebuild(self, variant: &HomeRebuildVariant) -> Result<()> {
        ...
        let (out_path, _tempdir_guard): (
            ...
            let dir =
                tempfile::Builder::new().prefix("nh-home").tempdir()?;
        ...
        let username = env::var("USER")
            .map_err(|_| eyre!("Couldn't get username"))?;
        let home_dir = env::var("HOME")
            .map_err(|_| eyre!("Couldn't get home directory"))?;
        ...
        Installable::Flake {
            ...
            if !attribute.is_empty() {
               ...
               the configuration name (e.g., '.#{}')",
                    ...
                    attribute
                        .insert(0, String::from("homeConfigurations"));
          ...
          "Failed running nix eval to check for explicit configuration \
           '{config_name}'"
                ...
                if check_res.trim() == "true" {
                    debug!(
                        "Using explicit configuration from flag: {config_name:?}"
                    );
            ...
            if !found_config {
                ...
                for attr_name in
                    [format!("{username}@{hostname}"), username]
                {
          ...
          .wrap_err(format!(
            "Failed running nix eval to check for automatic configuration \
             '{attr_name}'"
                    ...
                    if check_res.trim() == "true" {
                        debug!(
                            "Using automatically detected configuration: {}",
                            attr_name
                        );
