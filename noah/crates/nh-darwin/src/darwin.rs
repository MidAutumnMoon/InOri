...
use args::{
    DarwinArgs, DarwinRebuildArgs, DarwinReplArgs, DarwinSubcommand,
};
use color_eyre::{
    Result,
    eyre::{Context, bail},
    ...
    pub fn run(self, elevation: ElevationStrategy) -> Result<()> {
        ...
        match self.subcommand {
            DarwinSubcommand::Switch(args) => {
                args.rebuild(&Switch, elevation)
            }
            DarwinSubcommand::Build(args) => {
                if args.common.ask || args.common.dry {
                    warn!(
                        "`--ask` and `--dry` have no effect for `nh darwin build`"
                    );
                }
...
impl DarwinRebuildArgs {
    fn rebuild(
        ...
        let (out_path, _tempdir_guard): (
            ...
            let dir =
                tempfile::Builder::new().prefix("nh-darwin").tempdir()?;
    ...
    match res {
        Installable::Flake {
            ...
            if attribute.is_empty() {
                ...
                && attribute[0] == "darwinConfigurations"
            {
                info!(
                    "Inferring hostname '{hostname_str}' for darwinConfigurations"
                );
                attribute.push(hostname_str.to_owned());
