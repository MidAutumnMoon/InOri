...
fn invalid_token_message(source: TokenSource<'_>) -> String {
    ...
    let fix_message = match source {
        ...
        TokenSource::Prompt => {
            "Paste a single-line token without embedded whitespace."
                .to_string()
        }
    };
...
fn write_token_file(path: &Path, token: &SecretString) -> Result<()> {
    ...
    temp_file.as_file().set_permissions(perms).with_context(
        || "failed to set secure permissions on temporary file",
    )?;
...
mod tests {
    ...
    fn empty_token_file_env_errors() {
        ...
        let err =
            token_store_path().expect_err("empty override should fail");
    ...
    fn default_token_path_uses_xdg_state_home() -> Result<()> {
        ...
        assert_eq!(
            dir.path().join("nh").join(TOKEN_FILE),
            token_store_path()?
        );
        Ok(())
    }
