...
pub fn run(json: bool, args: &args::IssuesArgs, github_config: &GithubConfig) -> Result<()> {
    ...
    if issues.is_empty() {
        println!(
            "No issues found for '{query}' updated in the last {days} days."
        );
        return Ok(());
    }
...
mod tests {
    use super::*;
    use crate::search::github::IssueState;
    ...
    fn issue_json_uses_plain_urls() -> Result<()> {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!("https://github.com/NixOS/nixpkgs/issues/43"))
        );
        Ok(())
    }
}
