...
use crate::search::{
    branches::BranchPlan,
    github::{
        BranchReachability, BranchReachabilityStatus, PullRequest,
        PullRequestState,
    },
};
...
fn colored_status(state: PullRequestState) -> String {
    match state {
        PullRequestState::Open => {
            format!("{}", Paint::new("open").fg(Color::Blue))
        }
...
fn print_branch_reachability(
    ...
    let summary_reachability = summary_targets
        .iter()
        .filter_map(|branch| {
            reachability_by_branch.get(branch.as_str()).copied()
        })
        .collect::<Vec<_>>();
    ...
    if reached.is_empty() && !unknown.is_empty() {
        ...
        let branches = reached
            .iter()
            .map(|branch| {
                format!("{}", Paint::new(branch).fg(Color::Green))
            })
            .collect::<Vec<_>>()
            .join(", ");
...
fn format_unknown_check(branch: &BranchReachability) -> String {
    let error = match &branch.status {
        ...
        | BranchReachabilityStatus::Missing => {
            return format!(
                "{}",
                Paint::new(&branch.branch).fg(Color::Yellow)
            );
        }
    };
