...
pub fn run(opts: &CliOpts, config: &Config) -> Result<()> {
    let elevate =
        requires_elevation(opts.bypass_root_check, &config.elevation)?;
    ...
    let target_generation = if let Some(gen_number) = opts.to {
    ...
    if let Err(error) = super::diff::run(&opts.diff, &generation_link) {
    ...
    // Instead of direct symlink operations, use a command with proper
    // elevation
